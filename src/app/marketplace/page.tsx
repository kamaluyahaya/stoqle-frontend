      ...
      <div className="mt-3">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            {/* Wishlist / Add to cart buttons removed for clarity */}
          </div>
        </div>
      </div>
    </motion.article>
...
function handleExploreClick() {
  // treat >= 1024px as desktop (matches tailwind lg)
  if (typeof window !== "undefined" && window.innerWidth >= 1024) {
    // scroll the desktop sidebar into view and add a quick highlight
    sidebarRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
    if (sidebarRef.current) {
      sidebarRef.current.classList.add("ring-2", "ring-slate-300");
      // remove after 700ms
      setTimeout(() => sidebarRef.current && sidebarRef.current.classList.remove("ring-2", "ring-slate-300"), 700);
      // for accessibility, move focus into the sidebar
      const firstBtn = sidebarRef.current.querySelector<HTMLElement>("button");
      firstBtn?.focus();
    }
  } else {
  ...
  return (
    <>
      <div className="min-h-screen bg-white max-w-7xl mx-auto my-20 px-4">
        ...
        <main className="max-w-7xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="mt-4 flex flex-col gap-3">
                {(categories.length ? categories : ["All"]).map((c) => (
                  <button
                    ...
                    className={`text-sm text-left px-3 py-2 rounded-xl w-full ${activeCategory === c ? "bg-slate-900 text-white" : "bg-white/50"}`}
                  >
                    {c}
                  </button>
                ))}
          ...
          <section className="lg:col-span-3">
            ...
            <div className="bg-gradient-to-r from-white to-slate-50 rounded-3xl p-6 shadow-lg border border-gray-100 flex items-center gap-6">
              <div className="flex-1">
                ...
                <div className="mt-6 flex gap-3">
                  ...
                  {/* Explore categories now opens mobile sheet */}
                  <button
            ...
            {/* Mobile quick category chips (visible on small screens) */}
              ...
              <div className="flex gap-2 overflow-x-auto py-2">
                {(categories.length ? categories : ["All"]).map((c) => (
                  <button
                    key={c}
                    onClick={() => setActiveCategory(c)}
                    className={`whitespace-nowrap px-3 py-2 rounded-full text-sm font-medium border ${activeCategory === c ? "bg-slate-900 text-white border-transparent" : "bg-white border-gray-100"}`}
                  >
                    {c}
                  </button>
                ))}
      ...
      {/* Mobile categories sheet (AnimatePresence + motion) */}
      <AnimatePresence>
        {showCategories && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full bg-white rounded-t-2xl shadow-2xl p-4 max-h-[70vh] overflow-auto"
              role="dialog"
              aria-modal="true"
            >
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-lg font-semibold">Categories</h4>
                <button onClick={() => setShowCategories(false)} aria-label="Close categories" className="p-2 rounded-xl">
                  <X />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-3">
                {(categories.length ? categories : ["All"]).map((c) => (
                  <button
                    ...
                    className={`text-sm text-left px-3 py-3 rounded-xl w-full ${activeCategory === c ? "bg-slate-900 text-white" : "bg-white/50"}`}
                  >
                    {c}
                  </button>
                ))}
